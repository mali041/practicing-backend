...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    ...
    },
    {
      $lookup: {
        ...
        foreignField: "_id",
        as: "watchHistory",
        // Adding more Pipelines for nesting joins.
        pipeline: [
          {
            $lookup: {
              from: "users",
          ...
          {
            // This is Only for get data easily in frontend. Otherwise we get data from loop or array indexing
            $addFields: {
              owner: {
                $first: "$owner",
              },
  ...
  if (!user?.length) {
    throw new ApiError(404, "Watch History does not exits");
  }
